...
cout << " " << (int)(m/bf_elem_count) << " " <<min_est << " " << res << " " << max_est <<" " << cut_off << " "; // << endl;
   ...
   //printf("Param: max_ext=%.2f, x==%.2f, y=%.2f, min_est=%.2f, cut==%.2f, res=%.2f \n", max_est, x,y,min_est, cut_off, res);
...
#ifndef _M_IX86 // allowing win32 shortcut
    for( uint32_t j=0; j<bf_size/8; j++) {
        hamminglg += _mm_popcnt_u64(b64[j]); //return the number of bits set to 1
    }
#endif
}
