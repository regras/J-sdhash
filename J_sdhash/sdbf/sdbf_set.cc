...
sdbf_set::compare_all(int32_t threshold) { 
    ...
    for (int i = 0; i < end ; i++) {
        for (int j = i; j < end ; j++) {
            if (i == j) continue;
            double* jc = (double*) malloc(sizeof(double*));
            double* jr = (double*) malloc(sizeof(double*));
            int32_t score = this->items.at(i)->compare(this->items.at(j),0, jr, jc);
            if (*jr > 0 || *jc > 0 || score > 0) {
                #pragma omp critical 
                {
                    cout << this->items.at(i)->name() << this->sep << this->items.at(j)->name() ;
                    cout.precision(4);
                    cout << this->sep << setw (3) << score << this->sep << *jr << this->sep << *jc << std::endl;
                }
            }
        }            
    }
        ...
        for (int j = i; j < end ; j++) {
            ...
            if (*jr > 0 || *jc > 0 || score > 0) {
                #pragma omp critical 
                {
                    ...
                    out.precision(4);
                    out << this->sep << setw (3) << score << this->sep << *jr << this->sep << *jc << std::endl;
                }
            }
        }            
    }
...
sdbf_set::compare_to(sdbf_set *other,int32_t threshold,uint32_t sample_size) {
    ...
    for (int i = 0; i < qend ; i++) {
        for (int j = 0; j < tend ; j++) {
            double* jc = (double*) malloc(sizeof(double*));
            double* jr = (double*) malloc(sizeof(double*));
            int32_t score = this->items.at(i)->compare(other->items.at(j),sample_size, jr, jc);
            if (*jr > 0 || *jc > 0 || score > 0) {
                #pragma omp critical 
                {
