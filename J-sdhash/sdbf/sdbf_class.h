...
public:
    ...
    int32_t compare(sdbf *other, uint32_t sample, double* jr, double* jc);
...
private:
   ...
   // static int32_t get_elem_count(sdbf *mine, uint64_t index) ;
    ...
    static int     sdbf_score( sdbf *sd_1, sdbf *sd_2, uint32_t sample, double* jr, double* jc);
    static double  sdbf_max_score( sdbf_task_t *task, int *f1, int *f2, int *fcommon);
