...
*/
int32_t
sdbf::compare( sdbf *other, uint32_t sample, double* jr, double* jc) {
    if (config->warnings)
        cerr << this->name() << " vs " << other->name() << endl;

    return sdbf_score( this, other, sample, jr, jc);
}
...
sdbf::to_string () const { // write self to stream
    ...
    if( !this->elem_counts) {
        ...
        hash << (int)strlen((char*)this->hashname) << ":" << this->hashname << ":" << this->orig_file_size << ":fnv1a_64:";    
